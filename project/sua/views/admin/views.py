...
class ApplicationsMergeView(BaseView, NavMixin):
    ...
    def serialize(self, request, *args, **kwargs):
        ...
        application_set = Application.objects.filter(deleted_at=None).order_by('created')# 获取所有申请,按时间的倒序排序
    ...
    def deserialize(self, request, *args, **kwargs):
        ...
        if 'activity_id' in request.data:
            ...
            activity = merge_applications[0].sua.activity
        #activity_suas = Sua.objects.filter(deleted_at=None, activity=activity).all()
        for i in range(len(merge_applications)):
